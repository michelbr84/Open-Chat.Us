...
-- First, create content_filters table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.content_filters (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  filter_name TEXT NOT NULL,
  filter_pattern TEXT NOT NULL,
  filter_type TEXT NOT NULL DEFAULT 'regex', -- 'regex', 'keyword', 'phrase'
  severity_level INTEGER NOT NULL DEFAULT 1, -- 1-5 scale
  action_type TEXT NOT NULL DEFAULT 'flag', -- 'flag', 'auto_remove', 'warn'
...
CREATE TABLE IF NOT EXISTS public.moderation_queue (
  ...
  author_id UUID,
  author_name TEXT,
  flagged_by_filter_id UUID REFERENCES public.content_filters(id),
...
-- Enhanced user moderation status with more granular tracking
CREATE TABLE IF NOT EXISTS public.user_moderation_status (
  user_id UUID NOT NULL PRIMARY KEY,
  status TEXT NOT NULL DEFAULT 'active', -- 'active', 'warned', 'muted', 'banned'
  reputation_score INTEGER NOT NULL DEFAULT 100,
  total_warnings INTEGER NOT NULL DEFAULT 0,
  total_infractions INTEGER NOT NULL DEFAULT 0,
  muted_until TIMESTAMP WITH TIME ZONE,
  banned_until TIMESTAMP WITH TIME ZONE,
  last_infraction_at TIMESTAMP WITH TIME ZONE,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.moderation_actions (
  ...
  action TEXT NOT NULL, -- 'warn', 'mute', 'ban', 'unmute', 'unban', 'reputation_adjust'
...
CREATE POLICY "Admins can view all moderation status" 
...
BEGIN
  ...
  FOR filter_result IN 
    SELECT cf.*, 
           CASE 
             WHEN cf.filter_type = 'regex' THEN content_text ~* cf.filter_pattern
  ...
  LOOP
    IF filter_result.matches THEN
      ...
      triggered_filters := triggered_filters || jsonb_build_object(
        ...
        'severity_level', filter_result.severity_level,
        'action_type', filter_result.action_type
      );
      
      -- Accumulate violation score
      violation_score := violation_score + filter_result.severity_level;
      
      -- Determine most severe action required
      IF filter_result.action_type = 'auto_remove' AND action_required != 'auto_remove' THEN
        action_required := 'auto_remove';
      ELSIF filter_result.action_type = 'warn' AND action_required = 'allow' THEN
        action_required := 'warn';
      ELSIF filter_result.action_type = 'flag' AND action_required = 'allow' THEN
